...
// Ye bas camera enable karne ke liye hai brower mei
    ...
    if (videoStarted) {
      const runObjectDetection = async () => {
        ...
        const showRoomAlert = () => {
          ...
          const randomTime = Math.floor(Math.random() * (600000 - 300000 + 1)) + 300000; // Random time lega between 5 to 10 min
          setTimeout(showRoomAlert, randomTime);
        };

        const initialRandomTime = Math.floor(Math.random() * (600000 - 300000 + 1)) + 300000; // Initial random time
